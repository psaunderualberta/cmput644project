...
def test_heuristic_parsing_binary_3():
    assert parse_heuristic("(* 1.3 rate)") == Binary(
        "mul", Number(1.3), Terminal("rate")
    )


# Depth > 1
def test_heuristic_parsing_deep_1():
    assert parse_heuristic("(+ 1.3 (* 1.0 rate))") == Binary(
        "plus", Number(1.3), Binary("mul", Number(1.0), Terminal("rate"))
    )


def test_heuristic_parsing_deep_2():
    assert parse_heuristic("(max (neg rate) (abs ack_count))") == Binary(
