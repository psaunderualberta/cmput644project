...
def test_heuristic_depth_13():
    assert Binary("plus", Terminal("rate"), Binary("mul", Terminal("rate"), Number(2.0))).depth() == 3

def test_heuristic_depth_14():
    assert Binary("max", Unary("neg", Terminal("rate")), Unary("abs", Terminal("ack_count"))).depth() == 3
